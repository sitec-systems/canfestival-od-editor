    ...
    def SetStatusBarText(self, selection, manager):
        if selection:
            ...
            if manager.IsCurrentEntry(index):
                ...
                self.Frame.HelpBar.SetStatusText("Subindex: 0x%02X" % subIndex,
                                                 1)
                ...
                if entryinfos["struct"] & OD_IdenticalSubindexes:
                    ...
                    struct = "ARRAY"
                text = "%s: %s entry of struct %s%s." % (name, category,
                                                         struct, number)
            ...
            if edititem:
                ...
                if profile not in ("None", "DS-301"):
                    ...
                    for text, indexes in self.Manager.GetCurrentSpecificMenu():
                        new_id = wx.NewId()
                        self.Frame.AddMenu.Append(help='',
                                                  id=new_id,
                                                  kind=wx.ITEM_NORMAL,
                                                  text=text)
                        self.Frame.Bind(wx.EVT_MENU,
                                        self.GetProfileCallBack(text),
                                        id=new_id)
                else:
...
#-------------------------------------------------------------------------------
    ...
    def AddMapVariable(self):
        ...
        if index:
            ...
            if dialog.ShowModal() == wx.ID_OK:
                result = self.Manager.AddMapVariableToCurrent(
                    *dialog.GetValues())
    ...
    def ShowErrorMessage(self, message):
        message = wx.MessageDialog(self.Frame, message, "Error",
                                   wx.OK | wx.ICON_ERROR)
        message.ShowModal()
        message.Destroy()
