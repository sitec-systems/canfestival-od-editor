...
CustomisableTypes = [(0x02, 0), (0x03, 0), (0x04, 0), (0x05, 0), (0x06, 0),
                     (0x07, 0), (0x08, 0), (0x09, 1), (0x0A, 1), (0x0B, 1),
                     (0x10, 0), (0x11, 0), (0x12, 0), (0x13, 0), (0x14, 0),
                     (0x15, 0), (0x16, 0), (0x18, 0), (0x19, 0), (0x1A, 0),
                     (0x1B, 0)]
...
MappingDictionary = {
    ...
    0x1010: {
        ...
        "values": [{
            ...
            "name": "Save Manufacturer Parameters %d[(sub - 3)]",
            "type": 0x07,
            "access": 'rw',
            "pdo": False,
            "nbmax": 0x7C
    ...
    0x1011: {
        ...
        "values": [{
            ...
            "name":
            "Restore Manufacturer Defined Default Parameters %d[(sub - 3)]",
            "type": 0x07,
            "access": 'rw',
            "pdo": False,
            "nbmax": 0x7C
    ...
    0x1028: {
        ...
        False,
        "values": [{
            "name": "Number of Consumed Emergency Objects",
            "type": 0x05,
            "access": 'ro',
            "pdo": False
        }, {
    ...
    0x1800: {
        ...
        "values": [{
            ...
            False,
            "default":
            "{True:\"$NODEID+0x%X80\"%(base+1),False:0x80000000}[base<4]"
        }, {
            "name": "Transmission Type",
            "type": 0x05,
            "access": 'rw',
...
def FindEntryName(index, mappingdictionary, compute=True):
    ...
    if base_index:
        ...
        if infos["struct"] & OD_IdenticalIndexes and compute:
            return StringFormat(infos["name"],
                                (index - base_index) / infos["incr"] + 1, 0)
        else:
            return infos["name"]
    return None
...
def FindEntryInfos(index, mappingdictionary, compute=True):
    ...
    if base_index:
        ...
        if copy["struct"] & OD_IdenticalIndexes and compute:
            copy["name"] = StringFormat(
                copy["name"], (index - base_index) / copy["incr"] + 1, 0)
        copy.pop("values")
        return copy
    return None
    ...
    if base_index:
        ...
        if struct & OD_Subindex:
            ...
            if struct & OD_IdenticalSubindexes:
                ...
                elif 0 < subIndex <= mappingdictionary[base_index]["values"][
                        1]["nbmax"]:
            ...
            if infos is not None and compute:
                infos["name"] = StringFormat(infos["name"],
                                             (index - base_index) / incr + 1,
                                             subIndex)
            return infos
    return None
...
def FindMapVariableList(mappingdictionary, Node, compute=True):
    ...
    for index in mappingdictionary.keys():
        if Node.IsEntry(index):
            for subIndex, values in enumerate(
                    mappingdictionary[index]["values"]):
                if mappingdictionary[index]["values"][subIndex]["pdo"]:
                    infos = Node.GetEntryInfos(
                        mappingdictionary[index]["values"][subIndex]["type"])
                    name = mappingdictionary[index]["values"][subIndex]["name"]
                    if mappingdictionary[index][
                            "struct"] & OD_IdenticalSubindexes:
                        values = Node.GetEntry(index)
                        for i in range(len(values) - 1):
                            computed_name = name
                            if compute:
                                computed_name = StringFormat(
                                    computed_name, 1, i + 1)
                            list.append(
                                (index, i + 1, infos["size"], computed_name))
                    else:
                        computed_name = name
                        if compute:
                            computed_name = StringFormat(
                                computed_name, 1, subIndex)
                        list.append(
                            (index, subIndex, infos["size"], computed_name))
    return list
...
def FindIndex(index, mappingdictionary):
    if index in mappingdictionary:
        ...
        listpluri = [
            idx for idx in list(mappingdictionary.keys())
            if mappingdictionary[idx]["struct"] & OD_IdenticalIndexes
        ]
        listpluri.sort()
...
class Node:
    ...
    def AddEntry(self, index, subIndex=None, value=None):
        if index not in self.Dictionary:
            if not subIndex:
                ...
                self.Dictionary[index] = [value]
                return True
        elif subIndex > 0 and type(
                self.Dictionary[index]) == list and subIndex == len(
    ...
    def SetParamsEntry(self,
        ...
        if index in self.Dictionary:
            ...
            if subIndex == None or type(
                    self.Dictionary[index]) != list and subIndex == 0:
                if comment != None:
    ...
    def RemoveEntry(self, index, subIndex=None):
        ...
        if index in self.Dictionary:
            if not subIndex:
                ...
                return True
            elif type(self.Dictionary[index]) == list and subIndex == len(
                    self.Dictionary[index]):
    ...
    def HasEntryCallbacks(self, index):
        ...
        if entry_infos and "callback" in entry_infos:
            ...
            if not getattr(self, "ParamsDictionary", False):
                self.ParamsDictionary = {}
            if index in self.Dictionary and index in self.ParamsDictionary and "callback" in self.ParamsDictionary[
                    index]:
    ...
    def AddMappingEntry(self,
        ...
        if index not in self.UserMapping:
            ...
            if subIndex == None:
                ...
                return True
        elif subIndex != None and subIndex == len(
                self.UserMapping[index]["values"]):
            if values == None:
                values = {}
        ...
        if index in self.UserMapping:
            if subIndex == None:
                if name != None:
                    ...
                    if self.UserMapping[index][
                            "struct"] & OD_IdenticalSubindexes:
                        self.UserMapping[index]["values"][1][
                            "name"] = name + " %d[(sub)]"
                    elif not self.UserMapping[index][
                            "struct"] & OD_MultipleSubindexes:
                ...
                return True
                    ...
                    self.UserMapping[index]["values"]) and values != None:
                if "type" in values:
                    if self.UserMapping[index][
                            "struct"] & OD_IdenticalSubindexes:
                        if self.IsStringType(self.UserMapping[index]["values"]
                                             [subIndex]["type"]):
                            if self.IsRealType(values["type"]):
                                ...
                                for i in range(len(self.Dictionary[index])):
                                    self.SetEntry(index, i + 1, 0)
                        elif self.IsRealType(self.UserMapping[index]["values"]
                                             [subIndex]["type"]):
    ...
    def CompileValue(self, value, index, compute=True):
        if isinstance(
                value,
            (StringType, UnicodeType)) and value.upper().find("$NODEID") != -1:
...
#-------------------------------------------------------------------------------
    ...
    def GetSubentryInfos(self, index, subIndex, compute=True):
        ...
        while not result and i < len(mappings):
            ...
            if result:
                result["user_defined"] = i == len(
                    mappings) - 1 and index >= 0x1000
            i += 1
