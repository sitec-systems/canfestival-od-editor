...
ENTRY_TYPES = {
    ...
    7: {
        ...
        "optional": [
            "OBJECTTYPE", "DEFAULTVALUE", "PDOMAPPING", "LOWLIMIT",
...
# an EDS file
SECTION_KEYNAMES = [
    "FILEINFO", "DEVICEINFO", "DUMMYUSAGE", "COMMENTS", "MANDATORYOBJECTS",
    "OPTIONALOBJECTS", "MANUFACTUREROBJECTS", "STANDARDDATATYPES",
    "SUPPORTEDMODULES"
]
...
def ParseCPJFile(filepath):
    ...
    for section_name, assignments in sections:
        ...
        if section_name.upper() in "TOPOLOGY":
            ...
            for assignment in assignments:
                    ...
                    if keyname.isalnum():
                        ...
                        if value.startswith("0x") or value.startswith("-0x"):
                            try:
                                ...
                                raise SyntaxError(
                                    _("\"%s\" is not a valid value for attribute \"%s\" of section \"[%s]\""
                                      ) % (value, keyname, section_name))
                        elif value.isdigit(
                        ...
                        nodedcfname_result = nodedcfname_model.match(
                            keyname.upper())

                        if keyname.upper() == "NETNAME":
                            if not is_string(computed_value):
                                raise SyntaxError(
                                    _("Invalid value \"%s\" for keyname \"%s\" of section \"[%s]\""
                                      ) % (value, keyname, section_name))
                            ...
                            if nodeid not in list(topology["Nodes"].keys()):
                                topology["Nodes"][nodeid] = {}
                            topology["Nodes"][nodeid][
                                "DCFName"] = computed_value
                        else:
                            raise SyntaxError(
                                _("Keyname \"%s\" not recognised for section \"[%s]\""
                ...
                # All lines that are not empty and are neither a comment neither not a valid assignment
                    ...
                    raise SyntaxError(
                        _("\"%s\" is not a valid CPJ line") %
                        assignment.strip())

            if "Number" not in list(topology.keys()):
                raise SyntaxError(
                    _("\"Nodes\" keyname in \"[%s]\" section is missing") %
                    section_name)

            if topology["Number"] != len(topology["Nodes"]):
                raise SyntaxError(
                    _("\"Nodes\" value not corresponding to number of nodes defined"
                      ))

            for nodeid, node in list(topology["Nodes"].items()):
                if "Present" not in list(node.keys()):
                    raise SyntaxError(
                        _("\"Node%dPresent\" keyname in \"[%s]\" section is missing"
                          ) % (nodeid, section_name))
        ...
        # In other case, there is a syntax problem into CPJ file
            ...
            raise SyntaxError(
                _("Section \"[%s]\" is unrecognized") % section_name)
...
def ParseEDSFile(filepath):
    ...
    for section_name, assignments in sections:
        ...
        index_objectlinks_result = index_objectlinks_model.match(
            section_name.upper())
        ...
        if section_name.upper() in SECTION_KEYNAMES:
            ...
            if section_name.upper() not in eds_dict:
                ...
                raise SyntaxError(
                    _("\"[%s]\" section is defined two times") % section_name)
        ...
        # Third case, section name is a subindex name
            ...
            index, subindex = [
                int(value, 16) for value in subindex_result.groups()
            ]
            # If index hasn't been referenced before, we add an entry into the dictionary
        ...
        for assignment in assignments:
            ...
            # Verify that line is a valid assignment
                ...
                if keyname.isalnum():
                    ...
                    if value.upper().startswith("$NODEID"):
                        try:
                            test = int(value.upper().replace("$NODEID+", ""),
                                       16)
                            computed_value = "\"%s\"" % value
                        except:
                            raise SyntaxError(
                                _("\"%s\" is not a valid formula for attribute \"%s\" of section \"[%s]\""
                                  ) % (value, keyname, section_name))
                    ...
                    if computed_value != "":
                        ...
                        if is_entry:
                            ...
                            elif not ENTRY_ATTRIBUTES[keyname.upper()](
                                    computed_value):
                                raise SyntaxError(
                                    _("Invalid value \"%s\" for keyname \"%s\" of section \"[%s]\""
                                      ) % (value, keyname, section_name))
            ...
            # All lines that are not empty and are neither a comment neither not a valid assignment
                ...
                raise SyntaxError(
                    _("\"%s\" is not a valid EDS line") % assignment.strip())
        ...
        if is_entry:
            ...
            if not keys.issuperset(required):
                ...
                if len(missing) > 1:
                    attributes = _("Attributes %s are") % _(", ").join(
                        ["\"%s\"" % attribute for attribute in missing])
                else:
                    attributes = _("Attribute \"%s\" is") % missing.pop()
                raise SyntaxError(
                    _("Error on section \"[%s]\":\n%s required for a %s entry")
                    % (section_name, attributes,
                       ENTRY_TYPES[values["OBJECTTYPE"]]["name"]))
            ...
            if not keys.issubset(possible):
                ...
                if len(unsupported) > 1:
                    attributes = _("Attributes %s are") % _(", ").join(
                        ["\"%s\"" % attribute for attribute in unsupported])
                else:
    ...
    if param.upper() in values:
        try:
            if values["DATATYPE"] in (0x09, 0x0A, 0x0B, 0x0F):
                ...
                if not isinstance(values[param.upper()],
                                  (IntType, LongType)) and values[param.upper(
                                  )].upper().find("$NODEID") == -1:
                    raise
        except:
            raise SyntaxError(
...
def GenerateFileContent(Node, filepath):
    ...
    fileContent += "NrOfRXPDO=%d\n" % len(
        [idx for idx in entries if 0x1400 <= idx <= 0x15FF])
    fileContent += "NrOfTXPDO=%d\n" % len(
        [idx for idx in entries if 0x1800 <= idx <= 0x19FF])
    ...
    try:
        ...
        if ProfileNb not in [0, 301, 302]:
            ...
            ProfilePath = os.path.join(
                os.path.split(__file__)[0], "config/%s.prf" % ProfileName)
            ...
            if os.path.isfile(ProfilePath):
                try:
                    ...
                    exec(
                        compile(
                            open(ProfilePath, "rb").read(), ProfilePath,
                            'exec'))
                    ...
                    if values["OBJECTTYPE"] in [2, 7]:
                        if values["OBJECTTYPE"] == 2:
                            ...
                            if values["DATATYPE"] != 0xF:
                                raise SyntaxError(
                                    _("Domain entry 0x%4.4X DataType must be 0xF(DOMAIN) if defined"
                                      ) % entry)
                        # Add mapping for entry
                        Node.AddMappingEntry(entry,
                        ...
                        Node.AddMappingEntry(
                            ...
                            values={
                                ...
                                values["DATATYPE"],
                                "access":
                                ACCESS_TRANSLATE[values["ACCESSTYPE"].upper()],
                                "pdo":
                                values.get("PDOMAPPING", 0) == 1
                            })
                            ...
                            if subindex in values["subindexes"]:
                                Node.AddMappingEntry(
                                    ...
                                    values={
                                        ...
                                        ["DATATYPE"],
                                        "access":
                                        ACCESS_TRANSLATE[values["subindexes"][
                                            subindex]["ACCESSTYPE"].upper()],
                                        "pdo":
                                        values["subindexes"][subindex].get(
                                            "PDOMAPPING", 0) == 1
                                    })
                ...
                # Second case, entry is an ARRAY or a RECORD
                    ...
                    if "SUBNUMBER" in values and values["SUBNUMBER"] > 0:
                        ...
                        for subindex in range(1, int(max_subindex) + 1):
                            ...
                            if subindex in values[
                                    "subindexes"] and "PARAMETERVALUE" in values[
                                        "subindexes"][subindex]:
                                value = values["subindexes"][subindex][
                                    "PARAMETERVALUE"]
                            elif subindex in values[
                                    "subindexes"] and "DEFAULTVALUE" in values[
                                        "subindexes"][subindex]:
                                value = values["subindexes"][subindex][
                                    "DEFAULTVALUE"]
                        ...
                        raise SyntaxError(
                            _("Array or Record entry 0x%4.4X must have a \"SubNumber\" attribute"
                              ) % entry)
        return Node
