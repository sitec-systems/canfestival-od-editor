...
#-------------------------------------------------------------------------------
    ...
    [ID_HTMLFRAME,
     ID_HTMLFRAMEHTMLCONTENT] = [wx.NewId() for _init_ctrls in range(2)]
    ...
    class HtmlFrame(wx.Frame):
        def _init_ctrls(self, prnt):
            wx.Frame.__init__(self,
                              ...
                              name='HtmlFrame',
                              parent=prnt,
                              pos=wx.Point(320, 231),
                              size=wx.Size(853, 616),
                              style=wx.DEFAULT_FRAME_STYLE,
                              title='')
            self.Bind(wx.EVT_CLOSE, self.OnCloseFrame)

            self.HtmlContent = UrlClickHtmlWindow(
                id=ID_HTMLFRAMEHTMLCONTENT,
                name='HtmlContent',
                parent=self,
                pos=wx.Point(0, 0),
                size=wx.Size(-1, -1),
...
class objdictedit(wx.Frame, NodeEditorTemplate):
    ...
    def _init_coll_FileMenu_Items(self, parent):
        parent.Append(id=wx.ID_NEW, kind=wx.ITEM_NORMAL, item='New\tCTRL+N')
        parent.Append(id=wx.ID_OPEN, kind=wx.ITEM_NORMAL, item='Open\tCTRL+O')
        parent.Append(id=wx.ID_CLOSE,
                      kind=wx.ITEM_NORMAL,
                      item='Close\tCTRL+W')
        parent.AppendSeparator()
        parent.Append(id=wx.ID_SAVE, kind=wx.ITEM_NORMAL, item='Save\tCTRL+S')
        ...
        parent.AppendSeparator()
        parent.Append(id=wx.ID_EXIT, kind=wx.ITEM_NORMAL, item='Exit')
        ...
        self.Bind(wx.EVT_MENU,
                  self.OnImportEDSMenu,
                  id=ID_OBJDICTEDITFILEMENUIMPORTEDS)
        self.Bind(wx.EVT_MENU,
                  self.OnExportEDSMenu,
    ...
    def _init_coll_EditMenu_Items(self, parent):
        parent.Append(id=wx.ID_REFRESH,
                      ...
                      item='Refresh\tCTRL+R')
        parent.AppendSeparator()
        parent.Append(id=wx.ID_UNDO, kind=wx.ITEM_NORMAL, item='Undo\tCTRL+Z')
        parent.Append(id=wx.ID_REDO, kind=wx.ITEM_NORMAL, item='Redo\tCTRL+Y')
        parent.AppendSeparator()
    ...
    def _init_ctrls(self, prnt):
        ...
        self.HelpBar = wx.StatusBar(id=ID_OBJDICTEDITHELPBAR,
                                    name='HelpBar',
                                    parent=self)
        ...
        if selected >= 0:
            ...
            if result:
                ...
                if isinstance(result, (StringType, UnicodeType)):
                    message = wx.MessageDialog(self, result, "ERROR",
                                               wx.OK | wx.ICON_ERROR)
                    message.ShowModal()
                    message.Destroy()
        if not find_index:
    ...
    def OnHelpCANFestivalMenu(self, event):
        ...
        if wx.Platform == '__WXMSW__':
            ...
            if (os.path.isfile(readerexepath)):
                os.spawnl(
                    os.P_DETACH, readerexepath, "AcroRd32.exe", '"%s"' %
                    os.path.join(ScriptDirectory, "doc", "manual_en.pdf"))
            else:
                message = wx.MessageDialog(
                    self,
                    "Check if Acrobat Reader is correctly installed on your computer",
            ...
            try:
                os.system(
                    "xpdf -remote CANFESTIVAL %s %d &" %
                    (os.path.join(ScriptDirectory, "doc/manual_en.pdf"), 16))
            except:
                message = wx.MessageDialog(
                    self,
                    "Check if xpdf is correctly installed on your computer",
                    "ERROR", wx.OK | wx.ICON_ERROR)
                message.ShowModal()
                message.Destroy()

    def OnAboutMenu(self, event):
        self.OpenHtmlFrame("About CAN Festival",
                           os.path.join(ScriptDirectory, "doc/about.html"),
                           wx.Size(500, 450))
    ...
    def OnCloseFrame(self, event):
        ...
        if not self.ModeSolo:
            ...
            dialog = wx.MessageDialog(
                self, "There are changes, do you want to save?",
                "Close Application", wx.YES_NO | wx.CANCEL | wx.ICON_QUESTION)
...
#-------------------------------------------------------------------------------
    ...
    def OnNewMenu(self, event):
        ...
        if dialog.ShowModal() == wx.ID_OK:
            ...
            result = self.Manager.CreateNewNode(name, id, nodetype,
            ...
            if isinstance(result, (IntType, LongType)):
                ...
                new_editingpanel.SetIndex(result)
                self.FileOpened.AddPage(new_editingpanel, "")
                self.FileOpened.SetSelection(self.FileOpened.GetPageCount() -
                                             1)
                self.EditMenu.Enable(ID_OBJDICTEDITEDITMENUDS302PROFILE, False)
                if "DS302" in options:
                    self.EditMenu.Enable(ID_OBJDICTEDITEDITMENUDS302PROFILE,
                                         True)
                self.RefreshBufferState()
                self.RefreshProfileMenu()
                self.RefreshMainMenu()
    ...
    def OnOpenMenu(self, event):
        ...
        if filepath != "":
            ...
            directory = os.getcwd()
        dialog = wx.FileDialog(self, "Choose a file", directory, "",
                               "OD files (*.odx)|*.odx|All files|*.*",
                               wx.FD_OPEN)
        if dialog.ShowModal() == wx.ID_OK:
            ...
            if os.path.isfile(filepath):
                ...
                if isinstance(result, int):
                    ...
                    self.FileOpened.SetSelection(
                        self.FileOpened.GetPageCount() - 1)
                    if self.Manager.CurrentDS302Defined():
                        self.EditMenu.Enable(
                            ID_OBJDICTEDITEDITMENUDS302PROFILE, True)
                    else:
                        self.EditMenu.Enable(
                            ID_OBJDICTEDITEDITMENUDS302PROFILE, False)
                    self.RefreshEditMenu()
                    self.RefreshBufferState()
                    self.RefreshProfileMenu()
                    self.RefreshMainMenu()
    ...
    def SaveAs(self):
        ...
        if filepath != "":
            ...
            ), "%s.od" % self.Manager.GetCurrentNodeInfos()[0]
        ...
        if dialog.ShowModal() == wx.ID_OK:
            ...
            if os.path.isdir(os.path.dirname(filepath)):
                ...
                message = wx.MessageDialog(
                    self, f"[os.path.dirname(filepat) is not a valid folder!",
                    "Error", wx.OK | wx.ICON_ERROR)
                message.ShowModal()
                message.Destroy()
        dialog.Destroy()
        ...
        if dialog.ShowModal() == wx.ID_OK:
            ...
            if os.path.isfile(filepath):
                ...
                if isinstance(result, (IntType, LongType)):
                    ...
                    self.RefreshProfileMenu()
                    self.RefreshMainMenu()
                    message = wx.MessageDialog(self, "Import successful",
                                               "Information",
                ...
                message = wx.MessageDialog(
                    self, f"\"{filepath}\" is not a valid file!", "Error",
                    wx.OK | wx.ICON_ERROR)
                message.ShowModal()
                message.Destroy()
        dialog.Destroy()
    ...
    def OnExportEDSMenu(self, event):
        ...
        if dialog.ShowModal() == wx.ID_OK:
            ...
            if os.path.isdir(os.path.dirname(filepath)):
                ...
                if not result:
                    message = wx.MessageDialog(self, "Export successful",
                                               "Information",
                                               wx.OK | wx.ICON_INFORMATION)
                    message.ShowModal()
                    message.Destroy()
                ...
                message = wx.MessageDialog(
                    self,
                    f"\"{os.path.dirname(filepath)}\" is not a valid folder!",
                    "Error", wx.OK | wx.ICON_ERROR)
                message.ShowModal()
                message.Destroy()
        dialog.Destroy()
...
Error:
""" + str(e_type) + " : " + str(e_value), "Error", trcbck_lst)
    try:
...
def Display_Error_Dialog(e_value):
    message = wx.MessageDialog(None, str(e_value), "Error",
                               wx.OK | wx.ICON_ERROR)
    message.ShowModal()
    message.Destroy()
...
def format_namespace(d, indent='    '):
    return '\n'.join(
        ['%s%s: %s' % (indent, k, repr(v)[:10000]) for k, v in d.items()])
...
def AddExceptHook(path,
                  app_version='[No version]'):  #, ignored_exceptions=[]):
    def handle_exception(e_type, e_value, e_traceback):
        ...
        if str(e_value).startswith("!!!"):
            ...
            if result:
                ...
                output = open(
                    path + os.sep + "bug_report_" +
                    info['date'].replace(':', '-').replace(' ', '_') + ".txt",
                    'w')
                lst = list(info.keys())
                lst.sort()
                for a in lst:
