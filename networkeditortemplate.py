...
class NetworkEditorTemplate(NodeEditorTemplate):
    ...
    def RefreshNetworkNodes(self):
        ...
        if self.NodeList:
            new_editingpanel = EditingPanel(self.NetworkNodes, self,
                                            self.Manager)
...
#-------------------------------------------------------------------------------
    ...
    def RefreshBufferState(self):
        if self.NodeList is not None:
            ...
            if nodeID != None:
                nodename = "0x%2.2X %s" % (nodeID,
                                           self.Manager.GetCurrentNodeName())
            else:
...
#-------------------------------------------------------------------------------
    ...
    def OnAddSlaveMenu(self, event):
        ...
        if dialog.ShowModal() == wx.ID_OK:
            ...
            result = self.NodeList.AddSlaveNode(values["slaveName"],
                                                values["slaveNodeID"],
                                                values["edsFile"])
            if not result:
                new_editingpanel = EditingPanel(self.NetworkNodes, self,
    ...
    def OnRemoveSlaveMenu(self, event):
        ...
        dialog = wx.SingleChoiceDialog(self.Frame,
                                       _("Choose a slave to remove"),
                                       _("Remove slave"), slavenames)
        if dialog.ShowModal() == wx.ID_OK:
            ...
            if not result:
                ...
                if self.NetworkNodes.GetPageCount() > 0:
                    new_selection = min(current,
                                        self.NetworkNodes.GetPageCount() - 1)
                    self.NetworkNodes.SetSelection(new_selection)
                    if new_selection > 0:
                        self.NodeList.SetCurrentSelected(
                            slaveids[new_selection - 1])
                self.RefreshBufferState()
            else:
                self.ShowErrorMessage(result)
        dialog.Destroy()
