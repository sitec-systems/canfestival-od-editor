...
# ^^^^^^^^^^^^^
#
def catPO(applicationDirectoryPath,
          listOf_extraPo,
            ...
            if os.path.exists(langPOfileName):
                ...
                if targetDir is None:
                    ...
                    cmd = "msgfmt --output-file=%s/%s.mo %s_%s.po.cat" % (
                        mo_targetDir, applicationName, applicationName,
                        langCode)
                    if verbose: print cmd
                    os.system(cmd)
    os.chdir(currentDir)
...
# ^^^^^^^^^^^^^^^
#
def makeMO(applicationDirectoryPath,
           targetDir='./locale',
           applicationDomain=None,
           verbose=0,
           forceEnglish=0):
...
if __name__ == "__main__":
    ...
    try:
        optionList, pargs = getopt.getopt(sys.argv[1:], validOptions,
                                          validLongOptions)
