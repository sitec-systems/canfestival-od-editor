...
def GetValidTypeInfos(typename, items=[]):
    if typename in internal_types:
        ...
        if result:
            ...
            if values[0] == "UNSIGNED" and int(
                    values[1]) in [i * 8 for i in range(1, 9)]:
                typeinfos = ("UNS%s" % values[1], None, "uint%s" % values[1],
                             True)
            elif values[0] == "INTEGER" and int(
                    values[1]) in [i * 8 for i in range(1, 9)]:
                typeinfos = ("INTEGER%s" % values[1], None,
                             "int%s" % values[1], False)
            elif values[0] == "REAL" and int(values[1]) in (32, 64):
                typeinfos = ("%s%s" % (values[0], values[1]), None,
                             "real%s" % values[1], False)
                ...
                raise ValueError(
                    _("""!!! %s isn't a valid type for CanFestival.""") %
                    typename)
            if typeinfos[2] not in ["visible_string", "domain"]:
                internal_types[typename] = typeinfos
        else:
            raise ValueError(
                _("""!!! %s isn't a valid type for CanFestival.""") % typename)
    return typeinfos
...
def GetTypeName(Node, typenumber):
    ...
    if typename is None:
        raise ValueError(
            _("""!!! Datatype with value "0x%4.4X" isn't defined in CanFestival."""
              ) % typenumber)
    return typename
...
def GenerateFileContent(Node, headerfilepath, pointers_dict={}):
    ...
    for index in rangelist:
        ...
        if result:
            ...
            internal_types[rangename] = (typeinfos[0], typeinfos[1],
                                         "valueRange_%d" % num)
    ...
    for index in listIndex:
        ...
        if not isinstance(values, ListType):
            ...
            if entry_infos["struct"] & OD_IdenticalSubindexes:
                ...
                if index in variablelist:
                    texts["name"] = UnDigitName(FormatName(
                        entry_infos["name"]))
        ...
        for subIndex in range(len(values)):
            ...
            if subIndex == 0:
                ...
                if entry_infos["struct"] & OD_MultipleSubindexes:
                    ...
                    name = FormatName("%s_obj%04X" %
                                      (texts["NodeName"], texts["index"]))
            elif entry_infos["struct"] & OD_IdenticalSubindexes:
                if index in variablelist:
                    name = "%s[%d]" % (FormatName(
                        entry_infos["name"]), subIndex - 1)
                else:
                    name = "%s_obj%04X[%d]" % (texts["NodeName"],
                                               texts["index"], subIndex - 1)
            else:
                if index in variablelist:
                    name = FormatName(
                        "%s_%s" %
                        (entry_infos["name"], subentry_infos["name"]))
                else:
                    name = "%s_obj%04X_%s" % (
                        texts["NodeName"], texts["index"],
            ...
            if typeinfos[2] == "visible_string":
                if params_infos["buffer_size"] != "":
                    ...
                    sizeof = str(
                        max(len(values[subIndex]), default_string_size))
            elif typeinfos[2] == "domain":
                sizeof_param = FormatName(
            ...
            strIndex += "                       { %s%s, %s, %s, (void*)&%s, NULL }%s\n" % (
                subentry_infos["access"].upper(), save, typeinfos[2], sizeof,
                UnDigitName(name), sep)
...
""" % texts
    if texts["heartBeatTimers_number"] > 0:
        ...
        initializer = "{TIMER_NONE" + ",TIMER_NONE" * (
            texts["heartBeatTimers_number"] - 1) + "}"
...
s_PDO_status %(NodeName)s_PDO_status[%(maxPDOtransmit)d] = {""" % texts
    ...
    fileContent += ",".join(
        ["s_PDO_status_Initializer"] * texts["maxPDOtransmit"]) + """};
"""
...
def GenerateFile(filepath, node, pointers_dict={}):
    try:
        ...
        content, header = GenerateFileContent(node,
                                              os.path.split(headerfilepath)[1],
                                              pointers_dict)
